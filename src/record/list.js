 ...
 * @constructor
 */
const List = function (recordHandler, name, options) {
...
List.prototype.removeEntry = function (entry, index) {
  ...
  const currentEntries = this._record.get()
  const hasIndex = this._hasIndex(index)
  const entries = []
  let i
...
List.prototype._onReady = function () {
  ...
  for (let i = 0; i < this._queuedMethods.length; i++) {
...
List.prototype._afterChange = function () {
  ...
  const after = this._getStructure()
  const before = this._beforeStructure
  let entry
  let i
